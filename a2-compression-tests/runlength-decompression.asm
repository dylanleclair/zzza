...
;       - lo 7 bits indicate the repetitions
;
...
ROWS_ADDR = $9003               ; stores the number of rows on screen
    ...
    dc.w 12345 
    dc.b $9e, "4157", 0         ; jump to the end of the 'encoding' array
stubend
    dc.w 0
...
; DEF DECOMPRESSION
; we assume that the byte is at location $00 on 0 page
; and that the offset counter for the encoding array is at $01
    ...
    sta     $01                 ; keep another copy on the 0-page for indexing into the 'encoding' array
...
decompress
    ...
    tax                         ; put this length into the loop counter for 'draw'

    ; setup colour conditional (dealing with hi 1 bit)
    lda     $00                 ; get the byte back out of memory after we clobbered it with 0x7f
...
decompress_check
    ...
    ldx     $01                 ; grab our encoding offset from the zero-page

    ; get a byte to analyze
    lda     encoding,x          ; grab the byte stored at encoding+x
    sta     $00                 ; store the byte on zero-page for the decompress loop to access

    ; put the 'encoding' loop counter back in memory
    inx                         ; increment the offset by 1 for next time
    stx     $01                 ; store the encoding offset back on the zero-page
